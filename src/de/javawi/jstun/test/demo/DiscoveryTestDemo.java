...
public class DiscoveryTestDemo implements Runnable {
	...
	public void run() {
		try {
			//DiscoveryTest test = new DiscoveryTest(iaddress, "89.29.122.45", 3500);
                        DiscoveryTest test = new DiscoveryTest(iaddress, "89.29.122.45", 3478);
			...
			while (ifaces.hasMoreElements()) {
				...
				while (iaddresses.hasMoreElements()) {
					...
					if (Class.forName("java.net.Inet4Address").isInstance(iaddress)) {
                                                String ifaddr = NetworkInterface.getByInetAddress(iaddress).getName();
                                                if (ifaddr.startsWith("vmnet")) continue;
                                                if (ifaddr.startsWith("usb")==false) continue;
